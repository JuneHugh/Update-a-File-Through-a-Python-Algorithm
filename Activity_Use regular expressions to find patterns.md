...
For each component of the pattern, I'm not sure what would happen if it were missing. I'm assuming that it wouldn't match
...
Some examples of IP addresses that were extracted are 192.168.152.148 and 192.168.247.153 
Some examples of IP addresses that weren't extracted are 192.168.103.10654 and 1921.168.1283.75

These both seem to be valid IP addresses, but we're only looking for four sets of 3 numbers!
...
All IP addresses between 1 and infinity values per segment are printed to stdout.

This incldues even IP addresses with 4 or more values per segment
...
This includes all IP addresses with 1 to 3 values per segment and excludes invalid IP addresses (4 or more values per segment)
